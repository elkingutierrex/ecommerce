(function () {
  'use strict';
  var application = angular.module('appecommerce', [])

  application.controller('app', ['$scope', '$timeout', '$http', function ($scope, $timeout, $http) {

    var vm = $scope;
    // vm.swStartApp = false;

    vm.Init = function () {

      //  Objects
      vm.objFilter = {};
      vm.objUserLog = {
        id: null,
        name: null
      };
      vm.objModalHeader={
        modalShopping: 'Shopping cart'
        ,modalProduct: 'Product information'
        ,modalLogin : 'Login'
      };

      // Arrays
      vm.arrayGlobalUsers = [];
      vm.arrayGlobalShopping = [];
      vm.arrayUserShopping=[];



      //fuctions
      vm.modalShow = (id)=> {
        $('#'+id).modal('show');
      }

      vm.modalHide = (id)=> {
        $('#'+id).modal('hide');
      }

      vm.modalInfo = (item)=>{
        vm.objModal = {};
        vm.objModal = item;
      }

      vm.addShoppingItem = ()=> {
        if(!objUserLog){
          alertify.error('Primero debes iniciar sección para adicionar items a tus compras');
        }
        // if(!vm.array)

      }

      vm.filterLevels = (option, id) => {

        switch (option) {
          case 'category':
            vm.arraySubLevel = [];
            vm.categories.forEach((category) => {
              if (category.id == vm.objFilter.category) {
                vm.arraySubLevel = category.sublevels;
              }
            })
            break;
          case 'level':
            vm.arraySubLevel2 = [];
            vm.arraySubLevel.forEach((sublevel) => {
              if (sublevel.id == vm.objFilter.sublevel) {
                vm.arraySubLevel2 = sublevel.sublevels;
              }
            })
            break;
          case 'sublevel':
            console.log('Sublevel seleccionado')
            break;
        }
        vm.objFilter.sublevel_id = id;
        vm.filterProducts();
      }

      vm.filterProducts = () => {
        vm.filerProducts = angular.copy(vm.products)
        if (!vm.objFilter.category && !vm.objFilter.sublevel && !vm.objFilter.sublevel2) {
          alertify.info('No tienes filtro, visualizaras todos los productos!')
          return;
        }
        if (!vm.objFilter.sublevel_id) {
          alertify.info('filtro')
        } else {
          let filterProducts = [];
          filterProducts = vm.filerProducts.filter(prod => prod.sublevel_id == vm.objFilter.sublevel_id);
          if (filterProducts.length == 0) {
            vm.filerProducts = [];
            alertify.error('No se han encontrado item para ese filtro')
          } else {
            vm.filerProducts = filterProducts;
          }

        }
      }

      // vm.importar = function () {
      //   $http.get('products.json')
      //   .then(response => response.data)
      //   .catch(error => {
      //       $window.location.href = TreidConfigSrv.variables.url_redirect;
      //   });
      // }

      // vm.importar();

      // console.log(products);

      vm.products = [{
          "quantity": 308,
          "price": "$8,958",
          "available": false,
          "sublevel_id": 3,
          "name": "aute",
          "id": "58b5a5b1b6b6c7aacc25b3fb"
        },
        {
          "quantity": 891,
          "price": "$5,450",
          "available": true,
          "sublevel_id": 3,
          "name": "mollit",
          "id": "58b5a5b117bf36cf8aed54ab"
        },
        {
          "quantity": 698,
          "price": "$17,001",
          "available": false,
          "sublevel_id": 10,
          "name": "eiusmod",
          "id": "58b5a5b18607b1071fb5ab5b"
        },
        {
          "quantity": 449,
          "price": "$6,864",
          "available": true,
          "sublevel_id": 7,
          "name": "proident",
          "id": "58b5a5b13881e97291384813"
        },
        {
          "quantity": 736,
          "price": "$13,253",
          "available": false,
          "sublevel_id": 4,
          "name": "laboris",
          "id": "58b5a5b1b82dc20c7dd52260"
        },
        {
          "quantity": 850,
          "price": "$10,930",
          "available": false,
          "sublevel_id": 11,
          "name": "anim",
          "id": "58b5a5b1996384dbbc556718"
        },
        {
          "quantity": 644,
          "price": "$16,972",
          "available": true,
          "sublevel_id": 12,
          "name": "duis",
          "id": "58b5a5b17326875fe21aebc1"
        },
        {
          "quantity": 722,
          "price": "$6,904",
          "available": true,
          "sublevel_id": 9,
          "name": "sunt",
          "id": "58b5a5b1ac8306b5d1d3d5fd"
        },
        {
          "quantity": 620,
          "price": "$13,813",
          "available": true,
          "sublevel_id": 8,
          "name": "reprehenderit",
          "id": "58b5a5b1f9ebd51b88636467"
        },
        {
          "quantity": 297,
          "price": "$14,552",
          "available": true,
          "sublevel_id": 1,
          "name": "non",
          "id": "58b5a5b1a20b39205ba99c50"
        },
        {
          "quantity": 514,
          "price": "$11,042",
          "available": false,
          "sublevel_id": 1,
          "name": "dolor",
          "id": "58b5a5b11f741600608205ca"
        },
        {
          "quantity": 887,
          "price": "$17,532",
          "available": true,
          "sublevel_id": 13,
          "name": "dolor",
          "id": "58b5a5b1ef62cb996bb87c45"
        },
        {
          "quantity": 202,
          "price": "$6,174",
          "available": true,
          "sublevel_id": 13,
          "name": "enim",
          "id": "58b5a5b1a089200683925a83"
        },
        {
          "quantity": 700,
          "price": "$1,904",
          "available": false,
          "sublevel_id": 5,
          "name": "ad",
          "id": "58b5a5b12d7859f999ecccf1"
        },
        {
          "quantity": 343,
          "price": "$14,388",
          "available": false,
          "sublevel_id": 3,
          "name": "ullamco",
          "id": "58b5a5b1563189cbccc9adc7"
        },
        {
          "quantity": 435,
          "price": "$6,809",
          "available": false,
          "sublevel_id": 6,
          "name": "mollit",
          "id": "58b5a5b18f0d9e833ed59bcd"
        },
        {
          "quantity": 104,
          "price": "$9,341",
          "available": true,
          "sublevel_id": 1,
          "name": "aute",
          "id": "58b5a5b18041ec3e1f95de07"
        },
        {
          "quantity": 888,
          "price": "$3,793",
          "available": true,
          "sublevel_id": 5,
          "name": "magna",
          "id": "58b5a5b1b88a817e7b059cba"
        },
        {
          "quantity": 888,
          "price": "$13,110",
          "available": false,
          "sublevel_id": 7,
          "name": "aliqua",
          "id": "58b5a5b1c9a8564521717e20"
        },
        {
          "quantity": 724,
          "price": "$5,827",
          "available": true,
          "sublevel_id": 7,
          "name": "irure",
          "id": "58b5a5b1da0b4a52b88fc9f9"
        },
        {
          "quantity": 119,
          "price": "$14,457",
          "available": false,
          "sublevel_id": 3,
          "name": "nulla",
          "id": "58b5a5b1762f69a24c1b3026"
        },
        {
          "quantity": 104,
          "price": "$1,449",
          "available": false,
          "sublevel_id": 10,
          "name": "elit",
          "id": "58b5a5b10448a50924813126"
        },
        {
          "quantity": 296,
          "price": "$9,619",
          "available": false,
          "sublevel_id": 5,
          "name": "ea",
          "id": "58b5a5b1404bba7aeef39ef8"
        },
        {
          "quantity": 585,
          "price": "$18,849",
          "available": true,
          "sublevel_id": 5,
          "name": "occaecat",
          "id": "58b5a5b1abd8ab6045ca08cc"
        },
        {
          "quantity": 936,
          "price": "$2,843",
          "available": true,
          "sublevel_id": 8,
          "name": "quis",
          "id": "58b5a5b116513baf9ffc5444"
        },
        {
          "quantity": 399,
          "price": "$12,902",
          "available": false,
          "sublevel_id": 7,
          "name": "id",
          "id": "58b5a5b17ab1bf66a2c7b61b"
        },
        {
          "quantity": 735,
          "price": "$9,048",
          "available": false,
          "sublevel_id": 3,
          "name": "nostrud",
          "id": "58b5a5b1d99c26ea37cb738c"
        },
        {
          "quantity": 116,
          "price": "$1,482",
          "available": false,
          "sublevel_id": 1,
          "name": "reprehenderit",
          "id": "58b5a5b186acbed348266f1c"
        },
        {
          "quantity": 649,
          "price": "$5,673",
          "available": true,
          "sublevel_id": 9,
          "name": "voluptate",
          "id": "58b5a5b1b00bd95c14066c8b"
        },
        {
          "quantity": 839,
          "price": "$3,767",
          "available": false,
          "sublevel_id": 12,
          "name": "est",
          "id": "58b5a5b18f96294f880390c3"
        },
        {
          "quantity": 706,
          "price": "$17,417",
          "available": true,
          "sublevel_id": 4,
          "name": "aliqua",
          "id": "58b5a5b17e64c90c960eb3c7"
        },
        {
          "quantity": 566,
          "price": "$5,023",
          "available": false,
          "sublevel_id": 4,
          "name": "eu",
          "id": "58b5a5b1bba25641557222f1"
        },
        {
          "quantity": 331,
          "price": "$9,972",
          "available": false,
          "sublevel_id": 7,
          "name": "Lorem",
          "id": "58b5a5b174071d8ab7ca7ee1"
        },
        {
          "quantity": 975,
          "price": "$10,987",
          "available": false,
          "sublevel_id": 10,
          "name": "exercitation",
          "id": "58b5a5b1c2935d0602b9ccc5"
        },
        {
          "quantity": 340,
          "price": "$7,533",
          "available": true,
          "sublevel_id": 1,
          "name": "quis",
          "id": "58b5a5b17b4c47445bf6a234"
        },
        {
          "quantity": 543,
          "price": "$18,332",
          "available": false,
          "sublevel_id": 11,
          "name": "ad",
          "id": "58b5a5b1c2b40cb8d3976126"
        },
        {
          "quantity": 687,
          "price": "$15,006",
          "available": true,
          "sublevel_id": 8,
          "name": "proident",
          "id": "58b5a5b1b7b2382511968d1e"
        },
        {
          "quantity": 217,
          "price": "$8,651",
          "available": true,
          "sublevel_id": 1,
          "name": "id",
          "id": "58b5a5b11d6d4b1ec9db971c"
        },
        {
          "quantity": 466,
          "price": "$8,376",
          "available": false,
          "sublevel_id": 13,
          "name": "laborum",
          "id": "58b5a5b1e26c8f68850e2ceb"
        },
        {
          "quantity": 922,
          "price": "$8,732",
          "available": true,
          "sublevel_id": 8,
          "name": "non",
          "id": "58b5a5b13163243d012fb89a"
        },
        {
          "quantity": 553,
          "price": "$6,444",
          "available": false,
          "sublevel_id": 10,
          "name": "labore",
          "id": "58b5a5b1f3bf845c50e56244"
        },
        {
          "quantity": 859,
          "price": "$9,423",
          "available": true,
          "sublevel_id": 3,
          "name": "qui",
          "id": "58b5a5b1d24839bdfbc43800"
        },
        {
          "quantity": 842,
          "price": "$5,653",
          "available": true,
          "sublevel_id": 5,
          "name": "eiusmod",
          "id": "58b5a5b14a40d79ef80972fe"
        },
        {
          "quantity": 967,
          "price": "$15,468",
          "available": true,
          "sublevel_id": 11,
          "name": "mollit",
          "id": "58b5a5b17c36901b87f4f85a"
        },
        {
          "quantity": 102,
          "price": "$17,712",
          "available": true,
          "sublevel_id": 3,
          "name": "consequat",
          "id": "58b5a5b12cd2fb555a91652b"
        },
        {
          "quantity": 240,
          "price": "$2,192",
          "available": true,
          "sublevel_id": 6,
          "name": "enim",
          "id": "58b5a5b13bc95b01816c05b1"
        },
        {
          "quantity": 994,
          "price": "$18,523",
          "available": true,
          "sublevel_id": 9,
          "name": "occaecat",
          "id": "58b5a5b12bf26721f9eb7666"
        },
        {
          "quantity": 429,
          "price": "$12,909",
          "available": false,
          "sublevel_id": 7,
          "name": "elit",
          "id": "58b5a5b1ffde2a73491feb07"
        },
        {
          "quantity": 597,
          "price": "$14,970",
          "available": true,
          "sublevel_id": 3,
          "name": "sint",
          "id": "58b5a5b1c60cafd804b3bfc9"
        },
        {
          "quantity": 898,
          "price": "$9,091",
          "available": true,
          "sublevel_id": 4,
          "name": "in",
          "id": "58b5a5b148f556bae5c7d094"
        },
        {
          "quantity": 410,
          "price": "$5,525",
          "available": false,
          "sublevel_id": 11,
          "name": "excepteur",
          "id": "58b5a5b1ba6292fc572e9aaf"
        },
        {
          "quantity": 176,
          "price": "$15,789",
          "available": false,
          "sublevel_id": 13,
          "name": "irure",
          "id": "58b5a5b1f58fc52adfda923b"
        },
        {
          "quantity": 198,
          "price": "$15,502",
          "available": false,
          "sublevel_id": 8,
          "name": "dolore",
          "id": "58b5a5b137ecc41faaea9240"
        },
        {
          "quantity": 835,
          "price": "$10,050",
          "available": false,
          "sublevel_id": 2,
          "name": "ad",
          "id": "58b5a5b1b64b6d32f2c76262"
        },
        {
          "quantity": 144,
          "price": "$6,603",
          "available": false,
          "sublevel_id": 6,
          "name": "pariatur",
          "id": "58b5a5b12c5ceb44d1462659"
        },
        {
          "quantity": 977,
          "price": "$18,389",
          "available": false,
          "sublevel_id": 13,
          "name": "duis",
          "id": "58b5a5b13d958af8c892213b"
        },
        {
          "quantity": 806,
          "price": "$2,056",
          "available": false,
          "sublevel_id": 9,
          "name": "ut",
          "id": "58b5a5b11a61d1be3c7314ec"
        },
        {
          "quantity": 639,
          "price": "$4,262",
          "available": true,
          "sublevel_id": 11,
          "name": "aliquip",
          "id": "58b5a5b19b7d5b4d3f844ade"
        },
        {
          "quantity": 767,
          "price": "$10,651",
          "available": true,
          "sublevel_id": 10,
          "name": "aliqua",
          "id": "58b5a5b147af435dcecd4c7e"
        },
        {
          "quantity": 758,
          "price": "$16,235",
          "available": false,
          "sublevel_id": 3,
          "name": "id",
          "id": "58b5a5b1101a59975b4c30b2"
        },
        {
          "quantity": 546,
          "price": "$5,982",
          "available": true,
          "sublevel_id": 10,
          "name": "non",
          "id": "58b5a5b13f6e3eef1f16b817"
        },
        {
          "quantity": 592,
          "price": "$7,678",
          "available": false,
          "sublevel_id": 9,
          "name": "reprehenderit",
          "id": "58b5a5b18dbb3f02cb1629f5"
        },
        {
          "quantity": 945,
          "price": "$17,532",
          "available": false,
          "sublevel_id": 5,
          "name": "tempor",
          "id": "58b5a5b19212e84560bfd56a"
        },
        {
          "quantity": 307,
          "price": "$9,036",
          "available": false,
          "sublevel_id": 5,
          "name": "non",
          "id": "58b5a5b1c89c18d8ccc31f5a"
        },
        {
          "quantity": 378,
          "price": "$13,615",
          "available": false,
          "sublevel_id": 5,
          "name": "pariatur",
          "id": "58b5a5b136fe73b240c6bd7a"
        },
        {
          "quantity": 346,
          "price": "$7,621",
          "available": false,
          "sublevel_id": 1,
          "name": "reprehenderit",
          "id": "58b5a5b1cc7575c1f6358150"
        },
        {
          "quantity": 150,
          "price": "$14,842",
          "available": true,
          "sublevel_id": 12,
          "name": "aliquip",
          "id": "58b5a5b1be154bf75f25dec5"
        },
        {
          "quantity": 905,
          "price": "$17,876",
          "available": false,
          "sublevel_id": 7,
          "name": "veniam",
          "id": "58b5a5b144f5f4fa8c040060"
        },
        {
          "quantity": 717,
          "price": "$6,521",
          "available": false,
          "sublevel_id": 8,
          "name": "magna",
          "id": "58b5a5b1f5c3bb7704c16a2d"
        },
        {
          "quantity": 235,
          "price": "$2,504",
          "available": true,
          "sublevel_id": 12,
          "name": "tempor",
          "id": "58b5a5b1b37cd67f8371b4e3"
        }
      ]


      vm.categories = [{
          "id": 1,
          "name": "Bebidas",
          "sublevels": [{
            "id": 1,
            "name": "Gaseosas",
            "sublevels": [{
                "id": 2,
                "name": "Con azúcar"
              },
              {
                "id": 3,
                "name": "Sin azúcar"
              }
            ]
          }]
        },
        {
          "id": 2,
          "name": "Desayunos",
          "sublevels": [{
            "id": 4,
            "name": "Fake 1",
            "sublevels": [{
                "id": 5,
                "name": "Fake 2"
              },
              {
                "id": 6,
                "name": "Fake 3",
                "sublevels": [{
                  "id": 7,
                  "name": "Fake 4"
                }]
              }
            ]
          }]
        },
        {
          "id": 8,
          "name": "Almuerzos",
          "sublevels": [{
              "id": 9,
              "name": "Fake 5"
            },
            {
              "id": 10,
              "name": "Fake 6"
            }
          ]
        },
        {
          "id": 11,
          "name": "Vinos",
          "sublevels": [{
              "id": 12,
              "name": "Fake 8"
            },
            {
              "id": 13,
              "name": "Fake 9"
            }
          ]
        }
      ]



      vm.filerProducts = angular.copy(vm.products)





      // //truquiño: para actualizar los objetos del angular siempre dentro del timeout para que no reviente: by chepe
      // $timeout(() => {
      //   vm.$apply();
      // }, 2000);













    };
    vm.Init();
  }])
}());